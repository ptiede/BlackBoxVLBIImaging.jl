...
space never changes), and `fn` holds the once-compiled device score function and its
device posterior. The windowed run seeds `draws` with the sampler's real base-flat
draws as warmup produces them, so a refit reuses them and computes each score once;
without the cache every refit recompiles the score program and reloads the full window.
"""
_new_refit_cache() = (;
...
Fisher-divergence estimator (see [`_fisher_lowrank`](@ref)): draws AND scores in flat
space, one joint fit. `pilot` is an MCMC DiskStore directory (a run's `outbase`, or
`<outbase>/warmup` for the warmup log). Draws come from the run's own `flatdraws/` log
— the sampler's real base-flat positions — falling back to `inverse(asflat(post), θ)`
only for an external pilot without that log. The pilot must have sampled the SAME
model.
...
fit as seed directions (see [`_fisher_lowrank`](@ref)'s `carry`).
"""
function fit_preconditioner(
    ...
    # `augment`: carry the pilot's own preconditioner (its `transport.jls`) forward as
    # seed directions, so successive windows accumulate corrections instead of each short
    # log re-detecting only the widest and forgetting the rest.
    carry = nothing
    if augment
        tf = joinpath(
    ...
    # columns used are loaded, each scored ONCE across all refits via the cache, and the
    # score program compiles once — a cached refit costs seconds.
    ...
    for (k, idx) in enumerate(need)
        Zf[:, k] = get!(cache.draws, idx) do
            # The windowed refit writes the sampler's real base-flat draw here per warmup
            # step. Fall back to reconstructing from the stored angle only for an external
            # pilot with no such log: `inverse` returns the unit-radius representative,
            # which collapses an angle pair's radial direction.
            f = joinpath(pilot, "flatdraws", lpad(idx, 6, '0') * ".jls")
            isfile(f) ? deserialize(f)::Vector{Float64} :
