...
const GAIN_SCHEMES = Dict{String, NamedTuple}(
    "gain" => (parameterization = gain, kind = :jones,
        params = (:lg1, :gp1, :lgratμ, :lgratσ, :lgrat, :gprat, :gpratμ)),
    "gain_centered" => (parameterization = gain_centered, kind = :jones,
        params = (:lg1, :gp1, :lgrat, :gprat)),
    "gain_hier" => (parameterization = gain_hier, kind = :jones,
        params = (:lg1μ, :lg1σ, :lg1, :gp1, :lgratμ, :lgratσ, :lgrat, :gpratμ, :gpratσ, :gprat)),
    "gain_noratio" => (parameterization = gain_noratio, kind = :jones,
        params = (:lg, :gp)),
    "singlegain" => (parameterization = singlegain, kind = :single,
        params = (:lg, :gp)),
)

const LEAKAGE_SCHEMES = Dict{String, NamedTuple}(
    "none" => (parameterization = nothing, params = ()),
    "leakage_simple" => (parameterization = leakage_simple,
        params = (:d1re, :d1im, :d2re, :d2im)),
    "leakage_hier" => (parameterization = leakage_hier,
